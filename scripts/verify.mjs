...
const report = path.resolve(".local/validation", `${phase}.json`);
const previousStamp = new Date().toISOString().replace(/[:.]/g, "-");
for (const file of [log, report]) {
  if (existsSync(file)) copyFileSync(file, `${file}.${previousStamp}.previous`);
}
writeFileSync(
  log,
  `Verification ${phase} started ${new Date().toISOString()}\nNode ${process.version}; ${process.platform}\n`,
);
const commands = [
...
for (const [name, args] of commands) {
  ...
  writeFileSync(
    path.resolve(".local/validation", `${phase}.json`),
    JSON.stringify(
      { phase, timestamp: new Date().toISOString(), results },
      null,
      2,
    ),
  );
  if (code !== 0) {
    console.error(`Gate failed: ${name}. Later checks were not run.`);
    process.exit(code || 1);
  }
}
console.log(
  `All ${phase} gates passed. Evidence: .local/validation/${phase}.log`,
);
